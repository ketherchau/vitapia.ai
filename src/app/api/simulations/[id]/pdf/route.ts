...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const enrichedResponses = sim.results?.agent_responses?.map((r: { agent_id: string, choice: string, reasoning: string }) => ({
      ...r,
    ...
    enrichedResponses.forEach((r: Record<string, unknown>) => {
    ...
    if (enrichedResponses.length > 0) {
      ...
      topPct = ((sorted[0][1] / enrichedResponses.length) * 100).toFixed(1) + "%";
    }
    ...
    const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
      ...
      doc.text(`Total Responses: ${enrichedResponses.length} Synthetic Agents`);
      ...
      enrichedResponses.slice(0, 10).forEach((voice: Record<string, unknown>) => {
